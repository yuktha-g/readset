...
5. With the default `readset` scope, **every other file in the read set** is checked too.
   If you read `auth.py`, someone changed it, and you now try to write `models.py` based
   on what you saw, that's blocked as well. This is the difference between a file lock
   and a transaction: your write is validated against your assumptions, not just its target.

The fix is always one action: read the file again, then retry. The message says so.

**The guarantee, stated precisely:** no lost updates, and no writes based on stale reads.
readset does not provide full serialisability; two agents can still write-skew across files
that each read but neither wrote. That is the same trade PostgreSQL makes at
`REPEATABLE READ`, and it is the right one for v0.
...
```
readset init [--scope readset|target] [--user]   initialise this repo, install hooks
...
`--scope target` validates only the file being written (rule 5 off). Quieter; less safe.
...
PreToolUse Edit, readset scope       p50   80.4 ms   p95   89.8 ms
```
...
- **Whole-file granularity.** Two agents editing unrelated lines of one file conflict.
  Correct, but noisier than it needs to be. Hunk-level read sets are next.
...
- Hunk-level read sets, so unrelated edits to one file don't conflict
